...
serializeDecoderError size =
  extractId . reduceNoise . \case
    Binary.DecoderErrorCanonicityViolation lbl ->
      "couldn't decode due to internal constraint violations on '"
        <> lbl
        <> "': \
    ...
    Binary.DecoderErrorDeserialiseFailure lbl (Cbor.DeserialiseFailure offset hint) ->
      "invalid CBOR found at offset ["
        <> T.pack (show offset)
        <> "] while decoding a value of type '"
        <> lbl
        <> "': "
        <> T.pack (show hint)
    ...
    Binary.DecoderErrorLeftover lbl bytes ->
      "unexpected "
        ...
        <> " bytes found left after \
           \successfully deserialising a/an '"
        <> lbl
        <> "'"
    Binary.DecoderErrorSizeMismatch lbl expected actual
      | expected >= actual ->
          T.pack (show (expected - actual))
            <> " missing element(s) in a \
               \data-structure of type '"
            <> lbl
            <> "'"
    Binary.DecoderErrorSizeMismatch lbl expected actual ->
      T.pack (show (actual - expected))
        <> " extra element(s) in a \
           \data-structure of type '"
        <> lbl
        <> "'"
